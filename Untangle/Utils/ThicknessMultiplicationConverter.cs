...
namespace Untangle.Utils
{
	...
	public class ThicknessMultiplicationConverter : MarkupExtension, IValueConverter
	{
		#region IValueConverter Members

		/// <summary>
		/// Converts an object to <see cref="Thickness"/> and the multiplies it by a specific
		/// multiplier.
		/// </summary>
		/// <param name="value">The object that should be converted to <see cref="Thickness"/> and
		/// multiplied.</param>
		...
		/// </param>
		/// <returns>The result of the multiplication of the converted to <see cref="Thickness"/>
		/// <paramref name="value"/> and <paramref name="parameter"/>.</returns>
		public object Convert(object value, Type targetType, object parameter, CultureInfo culture)
		{
			Thickness left = (Thickness)value;
			double right = System.Convert.ToDouble(parameter, CultureInfo.InvariantCulture);

			Thickness result = new Thickness(left.Left * right, left.Top * right, left.Right * right, left.Bottom * right);
			return result;
		}
